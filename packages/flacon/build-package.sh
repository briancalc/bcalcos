...
EXPECTED_UPSTREAM_VERSION="13.0.2"

if [ "$UPSTREAM_VERSION" != "$EXPECTED_UPSTREAM_VERSION" ]; then
    echo "ERROR: This build script is for Flacon $EXPECTED_UPSTREAM_VERSION." >&2
    echo "       Changelog specifies Flacon $UPSTREAM_VERSION." >&2
    exit 1
fi

EXPECTED_SHA256="bbb85061d2ea3736184fd181d09e598278d28e970983cfb0d7b2f809776efd14"

DOWNLOAD_URL="https://github.com/flacon/flacon/archive/refs/tags/v${UPSTREAM_VERSION}.tar.gz"

BUILD_ROOT=$(mktemp -d "/tmp/${PACKAGE}-build-XXXXXX")
DOWNLOAD_FILE="$BUILD_ROOT/flacon-${UPSTREAM_VERSION}.tar.gz"
SOURCE_ARCHIVE="$BUILD_ROOT/${PACKAGE}_${UPSTREAM_VERSION}.orig.tar.gz"
SOURCE_DIR="$BUILD_ROOT/flacon-${UPSTREAM_VERSION}"
...
echo "==> Downloading Flacon source..."

curl -fL --retry 3 "$DOWNLOAD_URL" -o "$DOWNLOAD_FILE"
